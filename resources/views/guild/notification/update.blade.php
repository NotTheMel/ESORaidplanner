                ...
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="content">
                            ...
                            @if ($notification->type === \App\Notification\System\TelegramSystem::SYSTEM_ID)
                            ...
                            {{ Form::open(array('url' => route('notificationUpdate', ['slug' => $guild->slug, 'notification_id' => $notification->id]))) }}

                            Notification name (just for your
                            reference):{!! Form::text('name', $notification->name, ['class' => 'form-control', 'required']) !!}
                            <br>

                            @if ($notification->type === \App\Notification\System\DiscordSystem::SYSTEM_ID || $notification->type === \App\Notification\System\SlackSystem::SYSTEM_ID)
                                Webhook
                                url:{!! Form::text('url', $notification->url, ['class' => 'form-control', 'required']) !!}
                                <br>
                            @elseif ($notification->type === \App\Notification\System\TelegramSystem::SYSTEM_ID)
                                Chat ID
                                (Telegram):{!! Form::text('chat_id', $notification->chat_id, ['class' => 'form-control', 'required']) !!}
                                <br>
                            @endif
                            @if(\in_array($notification->call_type, \App\Notification\Configuration::RELATIVE_TIME_BASED_MESSAGES))
                                How many minutes before the event should this notification be sent?:
                                {!! Form::number('call_time_diff', $notification->call_time_diff / 60, array('class' => 'form-control')) !!}
                                <br>
                            @endif
                            @if(\in_array($notification->call_type, \App\Notification\Configuration::SIGNUP_BASED_MESSAGES))
                                Only send this message if the event has less signups than (leave blank or 0 to always
                                send
                                the message):
                                {!! Form::number('if_less_signups', $notification->if_less_signups, array('class' => 'form-control')) !!}
                                <br>
                                Only send this message if the event has this many signups or more (leave blank or 0 to
                                always
                                send
                                the message):
                                {!! Form::number('if_more_signups', $notification->if_more_signups, array('class' => 'form-control')) !!}
                                <br>
                            @endif

                            @if(\in_array($notification->call_type, \App\Notification\Configuration::TAG_BASED_MESSAGES))
                                ...
                                {!! Form::text('tags', implode(', ', $notification->tags()), ['class' => 'form-control']) !!}
                                <br>
                            @endif
                            @if(in_array($notification->call_type, \App\Notification\Configuration::DAILY_MESSAGES))
                                <div class="row">
                                    <div class="col-md-12">
                                    ...
                                    @if (Auth::user()->clock === 12)
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                Hour:{!! Form::number('hour', $notification->getDailyTriggerTime(Auth::user()->timezone)->format('g'), array('min' => '1', 'max' => '12', 'class' => 'form-control')) !!}
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                Minute:{!! Form::number('minute', $notification->getDailyTriggerTime(Auth::user()->timezone)->format('i'), array('min' => '0', 'max' => '59', 'class' => 'form-control')) !!}
                                                <br>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                Meridiem:{!! Form::select('meridiem', ['am' => 'am', 'pm' => 'pm'], $notification->getDailyTriggerTime(Auth::user()->timezone)->format('a'), array('class' => 'form-control')) !!}
                                                <br>
                                            </div>
                                        </div>
                                    @else
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                Hour:{!! Form::number('hour', $notification->getDailyTriggerTime(Auth::user()->timezone)->format('H'), array('min' => '0', 'max' => '23', 'class' => 'form-control')) !!}
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                            ...
                            @if(in_array($notification->call_type, \App\Notification\Configuration::TIMEZONE_BASED_MESSAGES))
                                Some guilds span over multiple timezones. This notification includes event start dates
                                and times.
                                {!! Form::select('timezones[]', \App\Utility\UserDateHandler::timeZones(), $notification->timezones(), array('class' => 'chosen-select form-control', 'multiple')) !!}
                            @endif
                            <br>
                            @if(in_array($notification->call_type, \App\Notification\Configuration::HAS_CUSTOM_TEXT_MESSAGE))
                            ...
                            @if ($notification->type === \App\Notification\System\DiscordSystem::SYSTEM_ID)
                                Use fancy Discord
                                messages: {!! Form::checkbox('has_embeds', 1, $notification->hasEmbeds()); !!}<br>
                            @endif
